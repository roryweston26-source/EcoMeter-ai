...
11. **Student/teacher routes — shipped as `student-access.json` (§5a); one open item left.** Google's student offer is the only unresolved one: its page sits behind a Google sign-in, so it's recorded as `unverified` and the panel says so. Resolving it needs Rory signed in with an .edu address reading the actual terms — don't take a number from a blog, since the ones describing it quote AI Pro at $28.99 against Google's own $19.99. Re-check GitHub's Copilot-for-students pause too; it was "temporarily paused" at the 2026-08-08 check.
