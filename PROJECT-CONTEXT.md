...
daily, and by how much. Built it — but the premise needed correcting first: only 3
of 31 plans can carry that number.** Detail in FRESHNESS **B9**.

**The ratio is computable exactly where a provider published both windows in the same
unit for the same thing. That is Z.ai and nobody else: 6.7× tighter, on all three
tiers.** Everything else resolves to one of five honest non-answers, and the split
between two of them is the part that matters.

⚠️ **"None published" and "not checked" are different claims.** Nine rows say the
provider publishes no longer window — every one backed by a dated `window_check` with
the pages listed. Seven say *we have not looked*, because nobody has. Collapsing
those two into one blank cell would put an absence in a provider's mouth on the
strength of our own backlog. E1g, one column over.

⚠️ **Two windows are only comparable when they bound the same thing.** Microsoft
publishes 25 agent tasks a month and 15 minutes of Vision a day. Dividing them would
produce a confident number about nothing, so that row states both and computes
neither.

**The column is mostly empty and that is the finding, the same way the Ceiling column
being empty was.** One provider of nine lets you see how bursty your plan is.

**`test-auditor.js` reached 106 checks.** Four destructive paths validated, including
the one that matters most: a `none` losing its sources. **The guard's first version
read the engine's fallback snapshot instead of the file and passed on an empty set —
the second time that exact trap has been hit in this file. The tell both times was a
count assertion reading 0.**
