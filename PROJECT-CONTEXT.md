...
11. **No student pricing anywhere, for a tool about to be marketed to students.** Verified 2026-08-08: **OpenAI has no consumer student discount** (only ChatGPT for Teachers, free for US K-12 educators through June 2027, and institutional ChatGPT Edu) — a genuine "not offered" finding worth stating. **Anthropic** likewise has no standalone student rate; Claude for Education is a partner-university programme. **Mistral publishes $5.99/mo for students**, already recorded in `prices.json` but unusable since Mistral isn't in the Auditor. **Google appears to run a student rate but it could not be confirmed on a Google-owned page** — the SEO blogs that describe it also state AI Pro's price as $28.99 when Google's own page says $19.99, so they fail the sourcing bar. Don't add any of this from a blog.
