...
11. **Student/teacher routes — shipped as `student-access.json` (§5a).** Google resolved: **the free 12-month student year expired 2026-04-30** (US) and 2025-12-09 (international), per Google's own `one.google.com/offer/studentoffer4` / `studentoffer6` terms pages. Its current plans and help pages mention no student offer. Recorded as `none`. **The trap to remember: expired offers outlive themselves** — search results and AI summaries still describe that free year in the present tense, and it's the most confidently-repeated false claim in this area. When re-verifying any offer, ask "what's the redemption deadline and has it passed", never "does an offer exist". Still open: re-check GitHub's Copilot-for-students pause (it was "temporarily paused" on 2026-08-08), and whether Google has since launched a discounted (not free) student rate — its student landing page is behind a sign-in, so that needs Rory signed in.
