...
- **Engine** (`recommend()`): for each provider you use, picks the cheapest tier that clears your **volume** (with a limit-hit headroom factor) and **model access**, then:
  ...
  - **volume-aware downgrade (any tier, not just free):** for every tier cheaper than the all-inclusive `fit`, if that tier clears your volume and offloading the models it *lacks* to pay-as-you-go API totals less, it recommends **"that tier + API"** — e.g. *free + API* for occasional Opus, or *ChatGPT Go + API* for occasional o3 (instead of jumping to Plus). Decided by real per-model API cost, so heavy premium use naturally fails the test and falls back to the inclusive tier — no arbitrary "occasional" threshold,
