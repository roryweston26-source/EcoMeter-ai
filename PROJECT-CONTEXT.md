...
16. **24 of 42 disclosure-matrix cells still have an empty `source_url`** — Amazon, xAI, OpenAI and Anthropic — while `_meta.methodology` claims "Every grade links to its sources." On the live page those cells render as plain text, so the claim is currently false. Google, Microsoft and Meta are now fully linked (18/42). **Either backfill the remaining four providers or soften the claim; backfilling is the honest one.** This is us failing the standard we grade others against, which is exactly what the mission says not to do.
...
**Disclosure matrix as of 2026-08-25** (`transparency-index.json`, `last_verified: 2026-08-25`): Google 🟢🟡🟡🟡🟡🟡 · Microsoft 🟢🔴🟡🟡🟡🟡 · Meta 🟡🔴🟡🟡🟡🟡 · Amazon 🔴🔴🔴🔴🔴🟡 · xAI/OpenAI/Anthropic 🔴 across the board. **Google is the benchmark and got further ahead**: 36 named locations with withdrawal, discharge *and* consumption, inside limited-assurance scope. Microsoft publishes 29 locations, withdrawal only, **outside** assurance. Meta publishes per-facility *electricity and emissions* but no per-site water. **Nobody splits AI from general load** — that column is 🔴 or 🟡 for all seven and is the real remaining gap.
